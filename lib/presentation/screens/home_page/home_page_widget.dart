...
class _WeatherAppHomePageState extends State<WeatherAppHomePage>
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        body: widget.showDataFromSavedCities == false
            ?
        BlocBuilder<InternateConnectivityBloc,
                InternateConnectivityState>(
                builder: (context, state) {
                  if (state is ConnectedState) {
                    return BlocConsumer<HomeControllerBloc,
                        HomeControllerState>(
                      ...
                      builder: (context, state) {
                        if (state is CurrentCityDataLoaded) {
                          ...
                          return currentCityLoadedUI(weatherCityModel, upcomingDays);
                        }
                    ...
                    /// make request from local database
                    //final currentCityForecastBloc = BlocProvider.of<LoadCurrentCityWeatherBloc>(context);
                    //  currentCityForecastBloc.add(GetCurrentCityWeatherData());
                    /// complete the rest for your self for now to save time i will show lottie
            ...
            :
        savedCityUIWidget(upcomingDays),
      ),
    );
  }

  Stack savedCityUIWidget(List<String> upcomingDays) {
    return Stack(
              children: [
                  ...
                  child: ListView(
                    children: [
                      ...
                      BlocBuilder<GetDailyForecastBloc,
                          GetDailyForecastState>(builder: (context, states) {
  ...
  Stack currentCityLoadedUI(WeatherModel weatherCityModel, List<String> upcomingDays) {
    return Stack(
                          children: [
